...
const authenticate = (req, res, next) => {
  ...
  try {
    ...
    // Always check the current database account.
    // This means deactivated users cannot continue using an
    // already-issued JWT.
    ...
    if (!user) {
      return next(
        new AppError('User account no longer exists.', 401)
      );
    }
    ...
    return next(
      new AppError('Invalid token. Access denied.', 401)
    );
  }
};
